...
def extract_embedding(img):
    try:
        results = get_deepface().represent(
            img_path=img,
        ...
        if "Face could not be detected" in error_msg:
            return (
                None,
                "Face not detected. Please ensure good lighting, look directly at the camera, and remove anything covering your face."
            )
...
def verify_face():
    ...
    confidence = (
        max(0, 1 - (min_distance / THRESHOLD))
